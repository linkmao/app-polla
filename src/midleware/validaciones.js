...
// Validación de usuario autenticado mediante passport, es decir via frontend
...
const isTeamDiferent = (req, res, next) => {
    ...
    if (isDiferent) { next() } else {
        ...
        if (req.params.g != "FINAL") { res.redirect(`/groups/${req.params.g}`) }  // Si es de grupos, se renderiza nuevamente el grupo
        else { res.redirect('/finals') } // Si no, entonces es de la fase final
    }
}
